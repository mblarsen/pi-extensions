...
Installing only this bridge will not provide observational memory or Lean Context. The `lean-ctx` executable must also be available on `PATH`, and Pi must run on Node.js 22.5 or newer for the built-in `node:sqlite` module. Install all three packages:

```bash
pi install git:github.com/elpapi42/pi-observational-memory
pi install npm:pi-lean-ctx
pi install npm:@mblarsen/pi-observational-memory-leanctx-bridge
```
