...
type ContentMessageEntry = MessageEntry & {
	message: { role: string; content: unknown };
};

function isContentMessageEntry(entry: SessionEntry): entry is ContentMessageEntry {
	return entry.type === "message" && "role" in entry.message && "content" in entry.message;
}
...
function hasToolCall(entry: ContentMessageEntry): boolean {
	const content = entry.message.content;
	return Array.isArray(content) && content.some((part) => !!part && typeof part === "object" && (part as { type?: unknown }).type === "toolCall");
}

function findLastUserMessage(branch: SessionEntry[]): ContentMessageEntry | undefined {
...
function findLastAssistantMessage(branch: SessionEntry[]): { entry: ContentMessageEntry; targetId: string; unsafeToolCall: boolean } | undefined {
	for (let index = branch.length - 1; index >= 0; index--) {
		const entry = branch[index];
		if (!isContentMessageEntry(entry) || entry.message.role !== "assistant") {
			continue;
		}
