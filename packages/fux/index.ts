...
function maybeFuxForkMetadata(entry: unknown): (FuxForkMetadata & JsonObject) | undefined {
	const data = fuxData(entry);
	return isFuxForkMetadata(data) ? data : undefined;
}

function fuxWidgetVisibility(entry: unknown): boolean | undefined {
	const data = fuxData(entry);
...
function fuxData(entry: unknown): JsonObject | undefined {
	if (!entry || typeof entry !== "object") return undefined;
	const object = entry as JsonObject;
	if (object.type !== "custom") return undefined;
	if (object.customType !== FUX_CUSTOM_TYPE) return undefined;
...
async function doFux(pi: ExtensionAPI, ctx: ExtensionContext, prompt?: string): Promise<void> {
	const currentSessionFile = ctx.sessionManager.getSessionFile();
	if (!currentSessionFile) {
